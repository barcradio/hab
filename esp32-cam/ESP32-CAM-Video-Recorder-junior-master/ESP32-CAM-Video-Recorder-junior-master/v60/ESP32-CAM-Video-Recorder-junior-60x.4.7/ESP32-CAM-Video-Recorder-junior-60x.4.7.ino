...
int IncludeInternet = 1;      // 0 for no internet, 1 for time only, 2  WiFiMan, 3 ssid in file, 4 ssid in file default off, 5 host mode
...
bool init_wifi() {
  ...
  if (IncludeInternet == 3 || IncludeInternet == 4 || IncludeInternet == 5) {
...
void scan_wifi() {
  ...
  if (n == 0) {
      ...
      Serial.println("Nr | SSID                             | RSSI (dBm) | RSSI (%) | CH | Encryption");
      for (int i = 0; i < n; ++i) {
          ...
          Serial.print(" | ");
          Serial.printf("%d", dBmtoPercentage(WiFi.RSSI(i)));
          Serial.print(" | ");
...
void setup() {
  ...
  Serial.print("AP:>>>>>>>>>>>>>>>>>>>>> "); Serial.println(appass);
  ...
  Serial.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
  Serial.println("You do not have wifi - no streamning, no file manager");
  ...
  if ( !InternetOff && IncludeInternet == 1) {
    Serial.printf("Time Setting done. Shutting off WiFi now \n\n");
    delay(1000);
    WiFi.disconnect();
    InternetOff = true;
  }
